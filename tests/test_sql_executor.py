...
def test_execute_with_string_literal():
    ...
    assert all(r["status"] == "aprovado" for r in rows)
...
def test_parametrize_string():
    _, params = SQLExecutor(SCHEMA)._parametrize("SELECT * FROM pedidos WHERE status = 'aprovado'")
    assert "p0" in params and params["p0"] == "aprovado"

def test_parametrize_no_literals():
    _, params = SQLExecutor(SCHEMA)._parametrize("SELECT COUNT(*) FROM clientes WHERE ativo = 1")
    assert params == {}
